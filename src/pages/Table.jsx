...
function Table({getEvent, eventDetails, setEventDetails }) {
  ...
  function comp(a , b){
    ...
    return (new Date(a.start.dateTime).getDate()) < (new Date(b.start.dateTime).getDate()) ;
  ...
  const dateHandler = (e) => {
    ...
    const val = eventDetails.filter((item) => {
      ...
      return (new Date(item.start.dateTime).toLocaleDateString()) === (new Date(e.target.value).toLocaleDateString());
    })
    ...
    <div className="container lg:w-[1280px] border p-2 flex flex-col flex-start gap-1 rounded-[10px] mx-auto my-auto mt-30 mb-10 bg-[#f9f9f9]">
      ...
      <div className="flex py-[7px] px-[10px]  bg-[#f7f7f7] justify-between items-center rounded-md">
        ...
        <div className="flex items-center gap-2 text-[#818181] relative">
          ...
          <input value={date} onChange={dateHandler} className="px-1 border-[#BEBEBE] outline-none bg-transparent border rounded-md" type="date" />
      ...
      <div className="flex px-[14px] py-[10px] justify-between items-center self-stretch">
        ...
        <div className="flex item gap-[10px]">
          <div className="flex items-center gap-[4px] text-[#7f67f6]">
            <FaCalendarPlus />
            <p className="text-[12px] font-normal leading-normal">Add Events</p>
          </div>

          <button onClick={() => deleteEvents(eventIds , token)} className="flex items-center gap-[4px] text-[#FF2E3C]">
            <MdDelete />
      ...
      {data &&
        data.map((item) => {
          return <EventDetails eventIds={eventIds} setEventIds={setEventIds} item={item}></EventDetails>;
        })}
